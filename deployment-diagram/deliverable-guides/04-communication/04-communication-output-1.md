...
| Web Application | Data Collection & Processing | → | HTTPS | 파일·문서 업로드 전달 | 업로드 intake · 변경 업로드 라우팅 |
...
| Data Collection & Processing | Knowledge Base | → | Knowledge Write | CPG·임베딩 적재 | 쓰기 중심 접근 |

### private → external — Data Sources (2행)

| 출발 Node | 도착 Node | 방향 | 표기 | 목적 | 비고 |
|---|---|---|---|---|---|
| Data Collection & Processing | Code Repository | → | Git Protocol | 코드 저장소 수집 | 소스 코드·커밋·브랜치 |
| Data Collection & Processing | Issue Tracker + Docs/Wiki | → | REST / Upload | 문서·티켓·PR 수집 | 문서·티켓·PR·Wiki |
...
| Data Collection & Processing | Public LLM Provider / Local LLM | → | REST / HTTPS | 임베딩 생성 | 용도별 호출 |
...
| 표기 | 사용 위치 | 특성 |
|---|---|---|
| **HTTPS** | Actor 브라우저 진입 (Developer/Reviewer/PM·PO → Web Application), API 호출 (Web Application → Workflow & Integration, Web Application → Data Collection & Processing) | 단발성 요청-응답 |
| **IDE UI** | Developer → IDE Plugin | 로컬 IDE 내부 UI/명령 기반 진입 |
| **REST / HTTPS** | Foundation 연동 (Interactive Assistant/Analysis Engine/Data Collection & Processing → Public LLM Provider / Local LLM, Web Application/Workflow & Integration → Platform & Infra) | 외부 서비스 API 호출 |
...
| **Internal API** | Workflow & Integration → Analysis Engine | 내부 서비스 간 직접 호출 |
...
| **Knowledge Write** | Data Collection & Processing → Knowledge Base | `Knowledge Base`에 대한 쓰기 중심 접근 |
| **Git Protocol** | Data Collection & Processing → Code Repository | 코드 저장소 수집 전용 |
| **REST / Upload** | Data Collection & Processing → Issue Tracker + Docs/Wiki | 문서·티켓 수집 |
