...
| `shared-contracts/` | 여러 Feature가 공유하는 계약 기준 |
| `docs/architecture/`, `docs/adr/` | Feature를 넘어 유지되는 구조와 결정 근거 |

### Harness 개발 흐름

Harness는 산출물의 세부 작성법을 정의하지 않는다. 대신 어떤 산출물이 있어야 하고, 어느 Feature ID에 연결되어야 하며, Gate 전에 어떤 상태를 충족해야 하는지를 정의한다. Feature별 운영 데이터는 `feature-registry/`에 두고, `harness/`는 그 데이터를 읽어 Gate를 수행하는 규칙과 도구를 담는다.
...
| 단계 | Harness 기준 |
| --- | --- |
| 착수 전 확인 | `specs/<feature-id>/`의 SpecKit 산출물을 확인한다. 기존 완료 근거와 `feature-registry/<feature-id>.yml`이 있으면 함께 확인한다 |
| 작업 식별 | `tasks.md`에서 이번에 진행할 task와 목표를 확인한다 |
| 운영 등록부 초안 작성 | 신규 Feature이면 예상 코드 쓰기 범위, 필수 문서 범위, 공동 수정 영역, 필요한 자동 검사를 `feature-registry/<feature-id>.yml`에 먼저 적는다 |
| 개발 중 기록 | Gate 판단에 영향을 주는 변경, 공동 수정 영역 가능성, 생략한 검증과 사유, 민감 정보 처리 판단을 기록한다 |
| Gate 전 운영 등록부 정리 | 실제 수정한 코드 범위, 갱신한 문서 범위, 공동 수정 영역 여부, 필요한 자동 검사 기준에 맞춰 `feature-registry/<feature-id>.yml`을 갱신한다 |
| 완료 후 산출물 | 완료 근거를 작성하거나 갱신하고, 완료된 task 상태를 정리한다 |
| PR 준비 | PR 본문에 Feature ID, 작업 요약, 검증 결과, 공동 수정 영역 승인 근거를 적는다 |
| Gate 통과 | Harness가 `feature-registry/<feature-id>.yml`과 PR 본문을 기준으로 자동 검사와 검토자 확인을 수행한다 |
...
| 실패 유형 | 처리 기준 |
| --- | --- |
| 자동 검사 실패 | 실패한 검사 기준에 맞춰 Feature ID, 완료 근거, 민감 정보, 쓰기 범위, 공동 수정 영역 승인 상태를 수정하고 검사를 다시 실행 |
...
Feature 전용 계약은 다음 조건을 만족할 때 `shared-contracts/`로 승격한다.
...
| --- | --- | --- |
| 구현 전 | `shared-contracts/`에 공유 기준을 먼저 만들고 Feature 전용 계약은 참조만 남김 | 공유 기준 책임자와 사용처 검토가 없으면 산출물 기준 미충족 |
...
1. `shared-contracts/`에 공유 기준을 만들거나 기존 기준을 갱신한다
...
예를 들어 첫 Feature에서만 쓰던 `specs/<feature-id>/contracts/events.md`의 이벤트 envelope가 두 번째 Feature에서도 같은 의미로 필요해지면, 공통 구조는 `shared-contracts/events/event-envelope.md`로 올리고 기존 Feature 계약에는 해당 공유 기준 참조와 Feature별 차이점만 남긴다.
...
계약 의미가 바뀌면 먼저 `contracts/` 또는 `shared-contracts/`의 기준 문서를 고친다. 그다음 영향받는 `plan.md`, `tasks.md`, `quickstart.md`, 이미 실행된 `verification.md`, Feature 등록부를 확인한다. 다른 개발자나 다른 Feature가 참조하는 계약이면 계약 변경 제안, 영향 범위 확인, 관련 작업 재분해, PR 준비와 검토 흐름으로 처리한다. PR Gate에서 이 상태를 어떻게 표시할지는 Harness 운영 기준에서 다루지만, 이 문서는 어떤 산출물을 고쳐야 하는지까지 정한다.
...
산출물 사이의 불일치가 의심되거나 계약 변경이 다른 개발자, 다른 Agent 작업, 다른 Feature, 공유 계약 기준에 영향을 주면 `analyze` 단계로 교차 점검한다.
...
| `shared-contracts/` | 공유 계약 변경 시 사용처와 승인 범위 확인 |
| `docs/architecture/`, `docs/adr/` | 장기 구조 변경과 되돌리기 어려운 결정의 근거 확인 |

Harness 내부 구조, 필수 자동 검사 구현 방식, Feature 등록부 전체 schema는 Harness 운영 기준에서 다룬다.
