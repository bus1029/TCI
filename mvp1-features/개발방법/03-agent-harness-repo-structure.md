...
TCI 초기 구조는 제품 코드, SpecKit 산출물, 경계 기준, evidence, Harness를 분리해서 둔다. SpecKit 기본 흐름과 충돌하지 않도록 `specs/`, `shared-contracts/`, `docs/`, `evidence/`, `harness/`의 역할을 나눈다.
...
└─ harness/                           # Agent 개발 방식과 PR Gate 운영 레이어
   └─ features/                       # TCI feature 운영 인덱스
      └─ <feature-id>.yml             # specs, shared-contracts, evidence, 코드 쓰기 범위, 필수 문서 범위 연결
```

제품 코드는 `apps/`와 `services/`에 두고, 기능 설명과 검증 기준은 제품 코드 밖에서 추적한다. `specs/<feature-id>/contracts/` 하위 디렉터리는 모든 Feature에 항상 만들지 않고, 해당 Feature의 경계 기준을 문서화해야 할 때만 포함한다. 여러 Feature가 함께 쓰는 공유 계약 기준은 `shared-contracts/`에서 참조한다.
...
따라서 이 문서의 추천은 SpecKit 기본 경로를 유지하되, 여러 Feature가 공유하는 계약 기준은 `shared-contracts/`, 완료 판단 근거는 `evidence/`, 장기 의사결정은 `docs/adr/`로 분리한다는 뜻이다.
...
root에는 `contracts/` 대신 `shared-contracts/`를 둔다. 이 위치는 Feature별 복사본을 두는 곳이 아니라 여러 Feature가 공유하는 계약 기준을 두는 곳이다. 예를 들어 공통 error response, auth header, workspace id 규칙, event envelope, 공통 trace attribute, artifact 저장 규약처럼 반복 적용되는 기준만 `shared-contracts/`로 올린다.
...
`shared-contracts/`는 앞에서 정의한 공유 계약 기준의 예시 구조와 승격 조건을 정리하는 위치다.
...
공유 계약 기준은 구현 전에 고정하고, 구현 뒤에는 테스트와 evidence가 같은 기준을 검증했는지 확인한다.

### 공유 기준 승격 기준

Feature contracts를 `shared-contracts/`로 승격하는 기준은 다음과 같다.
...
- `features/`: Feature ID별 specs, shared-contracts, evidence, 코드 쓰기 범위, 필수 문서 범위를 연결하는 TCI 운영 인덱스
...
shared_contracts_path: shared-contracts
...
| `shared_contracts_path` | 선택 | 이 Feature가 참조하는 공유 계약 기준 경로 |
